...
function arrayReturn(objectArray = [], value = ''){
    if(value === 'key') return keyReturn(objectArray)
        else if(value === 'value') return valueReturn(objectArray)
}

console.log(arrayReturn([{name: 'Dima', age: 13}, {model: 'Camry'}], 'key'))
...
function changeElementsById(array = [], index = 0){
    if(index >= array.length) return
    const changedElement = array[index]
    array[index] = array[index + 1]
    array[index + 1] = changedElement
}
array = [9,8,0,4]
changeElementsById(array, 2)
console.log(array)
...
function nullToEnd(array = []){
    ...
    for (let index = 0; index < array.length; index++) {
        array[index] !== 0 ? newArray.push(array[index]) : nullArray.push(array[index])
    }
    return  array = newArray.concat(nullArray)
}
console.log(nullToEnd([0,0,1,0]))
